...
class Environment(_CommonEnvironment, dm_env.Environment):
    ...
    def step_spec(self):
        ...
        warnings.warn(
            "`step_spec` is deprecated, please use `reward_spec` and " "`discount_spec` instead.", DeprecationWarning
        )
    ...
    def step(self, action):
        ...
        if not physics_is_divergent:
            ...
            terminating = (
                self._task.should_terminate_episode(self._physics_proxy) or self._physics.time() >= self._time_limit
            )
        else:
            reward = 0.0
            discount = 0.0
    ...
    def _substep(self, action):
        self._hooks.before_substep(self._physics_proxy, action, self._random_state)
        self._physics.step()
    ...
    def get_detailed_reward_spec(self):
        return self._task.get_detailed_reward_spec(self._physics_proxy)
